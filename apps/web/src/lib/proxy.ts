...
export async function proxyMonitor() {
  try {
    const upstream = await fetch(new URL("/api/market-monitor", apiBaseUrl()), {
      cache: "no-store",
      signal: AbortSignal.timeout(5_000),
      headers: { Accept: "application/json" },
    });
    if (!upstream.ok) {
      return Response.json({ detail: "Monitoring indisponible." }, { status: 502 });
    }
...
export async function proxyIntegrations() {
  try {
    const upstream = await fetch(new URL("/api/integrations", apiBaseUrl()), {
      cache: "no-store",
      signal: AbortSignal.timeout(5_000),
      headers: { Accept: "application/json" },
    });
    if (!upstream.ok) {
      return Response.json({ detail: "Registre des intégrations indisponible." }, { status: 502 });
    }
    const body: unknown = await upstream.json();
    return Response.json(body, { headers: { "Cache-Control": "no-store" } });
  } catch {
