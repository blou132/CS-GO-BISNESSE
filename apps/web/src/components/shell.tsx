...
function ShellContent({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      ...
      <nav aria-label="Navigation principale">{navigation.map((nav) => <Link key={nav.href} href={nav.href} className={`nav-link ${pathname === nav.href ? "active" : ""}`} aria-current={pathname === nav.href ? "page" : undefined}><Icon name={nav.icon} /><span>{nav.title}</span>{pathname === nav.href ? <span className="active-pip" /> : null}</Link>)}</nav>
