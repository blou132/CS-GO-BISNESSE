...
export function ListingsTable({ rows }: { rows: ScannerRow[] }) {
  return <div className="table-scroll" tabIndex={0} role="region" aria-label="Annonces observées, tableau défilant"><table className="listings-table"><thead><tr><th scope="col">Skin / Exterior</th><th scope="col">Marché</th><th scope="col" className="numeric-cell">Prix original</th><th scope="col" className="numeric-cell">Prix EUR <span>référence</span></th><th scope="col" className="numeric-cell">Float</th><th scope="col" className="numeric-cell">Valeur estimée</th><th scope="col" className="numeric-cell">Liquidité</th><th scope="col" className="numeric-cell">Risque</th><th scope="col" className="numeric-cell">Profit potentiel</th><th scope="col" className="numeric-cell">ROI</th><th scope="col" className="numeric-cell">Score</th><th scope="col"><span className="sr-only">Détail</span></th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}>
    ...
    <td className="numeric-cell">{row.liquidity ?? "—"}{row.liquidity_category ? <small className="cell-note">{row.liquidity_category.replaceAll("_", " ")}</small> : null}</td>
    <td className={`numeric-cell ${row.risk_score !== null && row.risk_score >= 70 ? "risk-high" : ""}`}>{row.risk_score ?? "—"}<small className="cell-note">/100</small></td>
